...
refr = False
if time.time() - os.path.getmtime('/users/aod/.mwto_aca/') > 3600.:
    refr = True
try:
    datas = Data.DsaDatabase3(refresh_apdm=refr, path='/users/aod/.mwto_aca/',
                              allc2=False, loadp1=False)
except IOError:
    datas = Data.DsaDatabase3(path='/users/aod/.mwto_aca/',
                              allc2=False, loadp1=False)
...
scorer.to_sql('scorer_wto_aca', engine, index_label='SBUID',
              if_exists='replace')
print('scorer written')
dsa.inputs.to_sql('inputs_wto_aca', engine, index_label='Cycle',
                  if_exists='replace')
print('inputs written')
dsa.selection_df.to_sql('selection_wto_aca', engine, index_label='SBUID',
                        if_exists='replace')
print('selection written')
dsa.master_dsa_df.to_sql('master_wto_aca', engine, index_label='SBUID',
                         if_exists='replace')
print('master written')
dsa.obs_param.to_sql('staticparam_wto_aca', engine, index_label='SBUID',
                     if_exists='replace')
